  ...
  <div class="space-y-6">
    <div class="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-xl space-y-4">
      <div class="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h3 class="text-xs font-black uppercase tracking-widest text-chantilly">Communication Tools</h3>
          <p class="text-[10px] text-gray-500 uppercase">Generate BCC distribution lists from {{ showArchived ? 'Full Database' : 'Current Roster' }}</p>
        </div>
        ...
        <div class="flex items-center gap-3 bg-gray-50 dark:bg-black/20 p-2 rounded-lg border border-gray-100 dark:border-gray-800">
          ...
          <label for="coach-toggle" class="text-[10px] font-bold uppercase text-gray-400 cursor-pointer select-none">Include Coaching Staff</label>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
        <button @click="copyDistributionList('athletes')" 
                class="px-3 py-3 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition rounded-lg text-[10px] font-bold uppercase tracking-widest">
          Athletes Only
        </button>
        <button @click="copyDistributionList('parents')" 
                class="px-3 py-3 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition rounded-lg text-[10px] font-bold uppercase tracking-widest">
          Parents Only
        </button>
        <button @click="copyDistributionList('all')" 
                class="px-3 py-3 bg-chantilly text-white shadow-lg shadow-chantilly/20 rounded-lg text-[10px] font-bold uppercase tracking-widest">
          Entire Team + Parents
        </button>
      </div>
    </div>
...
onMounted(() => {
  ...
  // Listen to Staff (Coaches) for the list
  onSnapshot(collection(db, "authorized_coaches"), (snapshot) => {
    coachEmails.value = snapshot.docs.map(doc => doc.data().email).filter(e => e)
  })
})
...
const copyDistributionList = (type) => {
  ...
  // 1. Collect from currently visible roster
  filteredAthletes.value.forEach(athlete => {
    if (type === 'athletes' || type === 'all') {
      if (athlete.email) emails.push(athlete.email)
    }
    if (type === 'parents' || type === 'all') {
      if (athlete.parentEmail1) emails.push(athlete.parentEmail1)
      if (athlete.parentEmail2) emails.push(athlete.parentEmail2)
    }
  })

  // 2. Add Coaches if checked
  if (includeCoaches.value) {
    coachEmails.value.forEach(email => emails.push(email))
  }

  // 3. De-duplicate and Join
  const uniqueEmails = [...new Set(emails.map(e => e.trim().toLowerCase()).filter(e => e !== ''))]
  const listString = uniqueEmails.join('; ')

  navigator.clipboard.writeText(listString).then(() => {
    alert(`Success! Copied ${uniqueEmails.length} addresses to clipboard.`)
  }).catch(err => {
