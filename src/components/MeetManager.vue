<template>
  <div class="space-y-6">
    <button v-if="selectedMeet" @click="selectedMeet = null" class="text-chantilly font-bold text-xs uppercase mb-4 flex items-center gap-2 hover:opacity-70 transition">
        ...
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            <tr v-for="meet in meets" :key="meet.id" class="hover:bg-gray-50 dark:hover:bg-white/5 transition group">
              ...
              <td class="p-4">
                <button @click="selectedMeet = meet" class="font-bold hover:text-chantilly transition text-left">
                  {{ meet.name }}
                </button>
              </td>
    ...
    <div v-else class="space-y-8">
      <div class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-xl border border-chantilly/20">
        <div class="flex justify-between items-start">
          ...
          <div class="text-right">
            <span class="text-[10px] bg-chantilly/10 text-chantilly px-3 py-1 rounded-full font-bold uppercase tracking-widest italic">Lineup Builder</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        <button v-for="g in ['Mens', 'Womens']" :key="g" 
                ...
                :class="activeGender === g ? 'bg-chantilly text-white shadow-lg shadow-chantilly/20' : 'bg-white dark:bg-gray-800 text-gray-500 border border-gray-200 dark:border-gray-700'"
                class="px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-widest transition-all">
          {{ g }}
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="event in getEventsForSeason(selectedMeet.season)" :key="event" 
             class="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
          <div class="flex justify-between items-center mb-4">
            <h4 class="font-black text-sm uppercase tracking-tight text-gray-800 dark:text-gray-100">
              {{ activeGender }} {{ event }}
            </h4>
            <button class="text-[10px] font-bold text-white bg-chantilly px-2 py-1 rounded hover:opacity-80 transition uppercase">
              + Add
            </button>
          </div>
          
          <div class="space-y-2">
            <div class="text-[11px] text-gray-400 italic py-2 border-t border-gray-50 dark:border-gray-800/50">
...
// Templates as requested
const templates = {
  Winter: [
    "4x800m Relay", "55m Dash", "55m Hurdles", "1600m Run", "4x200m Relay", 
    "500m Dash", "1000m Run", "300m Dash", "3200m Run", "4x400m Relay", 
    "Shot Put", "Long Jump", "Triple Jump", "High Jump", "Pole Vault"
  ],
  Spring: [
    "4x800m Relay", "100m Dash", "100 / 110m Hurdles", "1600m Run", "4x100m Relay", 
    "400m Dash", "800m Run", "200m Dash", "3200m Run", "4x400m Relay", 
...
const deleteMeet = async (meet) => {
  if (confirm(`Delete ${meet.name} and all associated lineups?`)) {
