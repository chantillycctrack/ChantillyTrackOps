  ...
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div v-if="!editingMeet" class="bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800 space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2 space-y-2">
          <label class="text-[10px] font-black uppercase text-chantilly tracking-[0.2em] ml-1">Location / Address</label>
          <input v-model="newMeet.location" type="text" placeholder="e.g. 1971 University Blvd, Lynchburg, VA" class="input-field-comm" />
        </div>
        <div class="space-y-2">
          <label class="text-[10px] font-black uppercase text-chantilly tracking-[0.2em] ml-1">MileStat Meet Link</label>
          <input v-model="newMeet.milestatUrl" type="url" placeholder="https://va.milesplit.com/..." class="input-field-comm" />
        </div>
      </div>
    ...
    <div v-if="editingMeet" class="bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-2xl border-2 border-chantilly space-y-6">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h3 class="font-black uppercase text-lg text-gray-900 dark:text-white">
            Edit: <span class="text-chantilly">{{ editingMeet.name }}</span>
          </h3>
          <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Update Logistics & Events</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
        <div class="space-y-1">
          <label class="text-[9px] font-black uppercase text-gray-400 ml-1">Location Address</label>
          <input v-model="editingMeet.location" class="input-field-comm !py-2" />
        </div>
        <div class="space-y-1">
          <label class="text-[9px] font-black uppercase text-gray-400 ml-1">MileStat URL</label>
          <input v-model="editingMeet.milestatUrl" class="input-field-comm !py-2" />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
        <div v-for="(event, index) in editingMeet.events" :key="index" 
          ...
          <input v-model="event.name" class="flex-1 bg-transparent border-none text-xs font-bold text-gray-900 dark:text-white focus:ring-0" />
          <div class="flex items-center gap-2 border-l border-gray-200 dark:border-gray-700 pl-3">
             ...
             <input v-model.number="event.limit" type="number" placeholder="∞" class="w-12 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 text-[10px] p-1 text-center text-gray-900 dark:text-white outline-none focus:border-chantilly transition" />
          </div>
          <button @click="editingMeet.events.splice(index, 1)" class="text-red-400 hover:text-red-600 px-2 text-xl font-light">×</button>
        </div>
        <button @click="editingMeet.events.push({name: 'New Event', limit: null})" class="border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-3 text-[10px] font-black uppercase text-gray-400 hover:border-chantilly hover:text-chantilly transition">+ Add Custom Event</button>
      </div>
    ...
    <div v-if="!editingMeet" class="grid grid-cols-1 gap-4">
      <div v-for="meet in meets" :key="meet.id" 
           class="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4 transition-all hover:border-chantilly/30">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1">
               <a v-if="meet.location" :href="'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(meet.location)" 
                  target="_blank" class="text-[9px] font-black uppercase text-chantilly hover:underline">
                 📍 {{ meet.location }}
               </a>
               <span v-else class="text-[9px] font-black uppercase text-gray-500">📍 TBD</span>
               <span class="text-[9px] font-black uppercase text-gray-500">⚡ {{ meet.events?.length || 0 }} Events</span>
            </div>
          </div>
        </div>
        
        <div class="flex gap-2">
          <a v-if="meet.milestatUrl" :href="meet.milestatUrl" target="_blank" 
             class="px-4 py-2 text-[10px] font-black uppercase rounded-lg border border-chantilly/30 text-chantilly hover:bg-chantilly hover:text-white transition">
            MileStat
          </a>
...
const getAutoSeason = () => {
  ...
  if (month >= 7 && month <= 10) return 'Fall (XC)';
  if (month >= 11 || month <= 3) return 'Winter (Indoor)';
  if (month >= 4 && month <= 6) return 'Spring (Outdoor)';
  return '';
}
...
const saveEvents = async () => {
  ...
  await updateDoc(meetRef, { 
    events: editingMeet.value.events,
    location: editingMeet.value.location,
    milestatUrl: editingMeet.value.milestatUrl
  })
